...
public class WPTree implements WordPath {
    ...
    public List<String> findPath(String word1, String word2) {
        ...
        // checks
        if (!this.nw.dict.isWord(word2)) return null;
...
return null as no path exists
        ...
        WPTreeNode currWord;
        while (!queue.isEmpty()) {
            currWord = queue.remove();
            for (String s : this.nw.distanceOne(currWord.getWord(), true)) {
                if (!visited.contains(s)) {
                    visited.add(s);
                    WPTreeNode currWordNode = currWord.addChild(s);
                    queue.add(currWordNode);
                    if (s.equals(word2)) {
                        return currWordNode.buildPathToRoot();
                    }
                }
            }
        }
