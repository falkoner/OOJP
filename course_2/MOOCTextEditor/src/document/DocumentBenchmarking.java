	...
	public static void main(String [] args) {
		...
		for (int numToCheck = start; numToCheck < numSteps*increment + start; numToCheck += increment)
		{
            ...
            for (int i = 0; i < trials; i++)
              new BasicDocument(input).getFleschScore();
            System.out.print((System.currentTimeMillis() - timer) / (float) trials + "\t");

            timer = System.currentTimeMillis();
            for (int i = 0; i < trials; i++)
                new EfficientDocument(input).getFleschScore();
            System.out.println((System.currentTimeMillis() - timer) / (float) trials);
        }
