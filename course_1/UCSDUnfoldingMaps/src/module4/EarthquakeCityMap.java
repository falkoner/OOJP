	...
	private void printQuakes() 
	{
        ...
        for (Marker quake : quakeMarkers) {
            ...
            if (quake instanceof LandQuakeMarker) {
                counter = summary.getOrDefault(((LandQuakeMarker) quake).getCountry(), 0);
                summary.put(((LandQuakeMarker) quake).getCountry(), counter + 1);
            }
            else if (quake instanceof OceanQuakeMarker) {
                counter = summary.getOrDefault("ocean", 0);
                summary.put("ocean", counter + 1);

            }
        }
        summary.entrySet().stream()
                .filter(entry -> !entry.getKey().equals("ocean"))
                .sorted(Map.Entry.comparingByKey())
                .forEach(entry -> System.out.println(String.format("%s: %s", entry.getKey(), entry.getValue())));

        System.out.println("\nIn the ocean: " + summary.getOrDefault("ocean", 0));
    }
