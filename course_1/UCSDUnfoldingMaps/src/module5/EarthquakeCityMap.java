...
public class EarthquakeCityMap extends PApplet {
	...
	public void mouseClicked()
	{
        ...
        for (Marker quake : quakeMarkers) {
            if(quake.isInside(map, mouseX, mouseY)) {
                clickQuake((EarthquakeMarker) quake);
    ...
    private void clickCity(CityMarker city) {
        cityMarkers.forEach(marker -> marker.setHidden(true));
        city.setHidden(false);
        for (Marker quake : quakeMarkers) {
            double radius = ((EarthquakeMarker)quake).threatCircle();
            if (city.getDistanceTo(quake.getLocation()) > radius) {
                quake.setHidden(true);
            }
        }
    }

    private void clickQuake(EarthquakeMarker quake) {
        double radius = quake.threatCircle();
        quakeMarkers.forEach(marker -> marker.setHidden(true));
